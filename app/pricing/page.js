      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20 }}>
          {Object.entries(PLANS).map(([key, plan]) => (
            <div key={key} className="card" style={{ position: 'relative', overflow: 'visible', border: plan.popular ? '2px solid #1a1a2e' : undefined }}>
              {plan.popular && <div style={{ position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)', background: '#1a1a2e', color: 'white', padding: '4px 16px', borderRadius: 12, fontSize: 11, fontWeight: 600, letterSpacing: '0.03em', whiteSpace: 'nowrap', zIndex: 1 }}>RECOMMENDED</div>}
              <div className="card-body" style={{ padding: 28 }}>
                ...
                {plan.price === 0 ? (
                  ...
                  >
                    {loading === key ? 'Redirecting to checkout...' : `Subscribe — $${plan.price}/mo`}
                  </button>
                )}
              </div>
            </div>
